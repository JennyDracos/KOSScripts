...
Parameter runway is "KSC".
Parameter landingSpeed is 65.
Parameter approachSpeed is 160.
...
if runway = "KSC" {
	set east_geo to latlng(-0.0500685844665193,-74.5020656504963).
	set west_geo to latlng(-0.0485983135226979, -74.715357349566).
} else if runway = "KSCReverse" {
	set east_geo to latlng(-0.0485983135226979, -74.715357349566).
	set west_geo to latlng(-0.0500685844665193, -74.5020656504963).
}

//set east_ves to vessel("VASI east").
//set west_ves to vessel("VASI west").
//set runway_alt to body:altitudeof(west_ves:position).
set runway_alt to west_geo:terrainheight.

lock runway_vect to (east_geo:position - west_geo:position):normalized.
...
local aim_pos is east_geo:altitudeposition(0).
aim_geo_list:add(ship:body:geopositionof(aim_pos)).
aim_alt_list:add(0).
aim_spd_list:add(landingSpeed).
...
//  local aim_pos is west_ves:geoposition:altitudeposition(aim_alt) - (120 + 1500*arith_series)*runway_vect.
  local aim_pos is west_geo:altitudeposition(aim_alt) - (120 + 1500*arith_series)*runway_vect.
  local aim_geo is ship:body:geopositionof(aim_pos).
  aim_geo_list:add(aim_geo).
  aim_alt_list:add(aim_alt).
  if i = 0 { 
    aim_spd_list:add(landingSpeed * 1.1).
  } else if i = 1{
    aim_spd_list:add(landingSpeed * 1.2).
  } else {
    aim_spd_list:add(approachspeed).
  }
  set i to i+1.
...
when (ship:altitude - ship:geoposition:terrainheight) < 200 then {
  if not gear {
    gear on.
...
until user_quit or status="LANDED" or cur_aim_i < 0 {
  ...
  if cur_aim_i > 0 {
    ...
    if (east_geo:position - ship:position):mag < (west_geo:position - ship:position):mag {
