...
//if not (defined liborbitparam_def) { run liborbitparam. }
...
declare function getChangeInc {
	...
	local b3 to cos(i2).
	ptp("cos(xfrAng): " + (a1 * b1 + a2 * b2 + a3 * b3)).
	local xfrAng to arccos(a1 * b1 + a2 * b2 + a3 * b3).
	ptp("inclination transfer angle: " + xfrAng).
	local c1 to a2 * b3 - a3 * b2.
	local c2 to a3 * b1 - a1 * b3.
	local c3 to a1 * b2 - a2 * b1.
	local xfrLon to arctan2(c2, c1) + 90.
	print "c1: " + c1.
	if c1 > 0 { set xfrLon to xfrLon + 180. }
	local xfrLat to arctan2(c3, sqrt(c1^2 + c2^2)).
	print "xfrLon: " + xfrLon.
	print "xfrLat: " + xfrLat.
	local az1 is arcsin(cos(i1)/cos(xfrLat)).
	local az2 is arcsin(cos(i2)/cos(xfrLat)).
	print "az1: " + az1.
	print "az2: " + az2.
	if az2 < az1 { set xfrAng to -1 * xfrAng. }
	local ta_an to clamp360(xfrLon - ship:obt:argumentofperiapsis - ship:obt:lan).
	if clamp360(ta_an - ship:obt:trueanomaly) > 180 {
...
declare function getPhaseAngleAt1 {
	...
	if vang(fore, pos_tgt) > 90 { set angle to -1 * angle. }
	return angle.
}
