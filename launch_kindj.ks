...
local watchEngine is engineList[0].
local watchThrust is 50.
for engine in engineList {
	if engine:name = "RAPIER" { 
		set watchEngine to engine.
		set watchThrust to 50.
	} else if engine:name = "nfa-multimodal-25-1" {
		set watchEngine to engine.
		set watchThrust to 210.
	}
}
...
until ship:altitude > 070000 {
	...
	if mode = "pitch up"
	{
		...
		if mypitch <= 1 {
			...
			when watchEngine:thrust < watchThrust then {
				for engine in engineList {
					if engine:name = "RAPIER" or
					   engine:name:startswith("nfa-multimodal")
					{ EngineStateToggle(engine). }
					else if engine:name:contains("nebula") {
						engine:shutdown.
					}
				}
